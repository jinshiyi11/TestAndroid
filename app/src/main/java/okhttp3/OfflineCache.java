 ...
 */
public final class OfflineCache implements Closeable, Flushable {
    ...
    final InternalCache internalCache = new InternalCache() {
        @Override public Response get(Request request) throws IOException {
            return OfflineCache.this.get(request);
        }

        @Override public CacheRequest put(Response response) throws IOException {
            return OfflineCache.this.put(response);
        }

        @Override public void remove(Request request) throws IOException {
            OfflineCache.this.remove(request);
        }

        @Override public void update(Response cached, Response network) throws IOException {
            OfflineCache.this.update(cached, network);
        }

        @Override public void trackConditionalCacheHit() {
            OfflineCache.this.trackConditionalCacheHit();
        }

        @Override public void trackResponse(CacheStrategy cacheStrategy) {
            OfflineCache.this.trackResponse(cacheStrategy);
        }
    };
    ...
    OfflineCache(File directory, long maxSize, FileSystem fileSystem) {
    ...
    Response get(Request request) {
        ...
        try {
            entry = new OfflineCache.Entry(snapshot.getSource(ENTRY_METADATA));
        } catch (IOException e) {
    ...
    private CacheRequest put(Response response) throws IOException {
        ...
        OfflineCache.Entry entry = new OfflineCache.Entry(response);
        ...
        try {
            ...
            return new OfflineCache.CacheRequestImpl(editor);
        } catch (IOException e) {
            abortQuietly(editor);
            return null;
        }
    ...
    private void update(Response cached, Response network) {
        OfflineCache.Entry entry = new OfflineCache.Entry(network);
        DiskLruCache.Snapshot snapshot = ((OfflineCache.CacheResponseBody) cached.body()).snapshot;
        DiskLruCache.Editor editor = null;
        try {
    ...
    private final class CacheRequestImpl implements CacheRequest {
        ...
        public CacheRequestImpl(final DiskLruCache.Editor editor) throws IOException {
            ...
            this.body = new ForwardingSink(cacheOut) {
                @Override public void close() throws IOException {
                    synchronized (OfflineCache.this) {
                        if (done) {
                            return;
                        }
                        done = true;
    ...
    private static final class Entry {
        ...
        public Response response(DiskLruCache.Snapshot snapshot) {
            ...
            return new Response.Builder()
                    ...
                    .body(new OfflineCache.CacheResponseBody(snapshot, contentType, contentLength))
                    .handshake(handshake)
